...
function Toast({ message, show, error }: { message: string, show: boolean, error: boolean }) {
  return (
    <div
      className={`fixed top-10 sm:top-4 right-4 p-3 ${error ? 'bg-red-800' : 'bg-[#222831] dark:bg-[#FEFAF6]'} text-[#FEFAF6] dark:text-[#222831] text-sm rounded-lg shadow-lg transition-opacity duration-500 ${show ? "opacity-100" : "opacity-0"}`}
    >
      {message.split('\n').map((line, index) => (
        <span key={index}>
          {line}
...
export default function ContactForm() {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText('mykhaylopelykh@gmail.com')
      .then(() => {
  ...
  return (
    <div className={`container mx-auto pt-6 pb-6 md:pb-12 lg:pb-16 mt-16`} id='contact' ref={formRef}>
      ...
      <div className="flex flex-row justify-center mt-6">
        <span
          className="inline-flex items-center justify-center gap-1 rounded-full border border-[#222831] dark:border-[#FEFAF6] px-3 py-1 text-[#222831] dark:text-[#FEFAF6] cursor-pointer"
